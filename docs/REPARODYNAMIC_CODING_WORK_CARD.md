...
Status: SHADOW foundation candidate. No production activation, model spending, merge, deployment, or authority expansion is granted by this card.
...
- Maximum destination: SHADOW; a later explicitly approved CANARY may only select a verified SHADOW candidate.
...
7. CANARY returns only a deterministically verified proposal and the existing kernel still re-verifies it into SHADOW.
