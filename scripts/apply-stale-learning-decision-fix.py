...
kernel_old = '''      const request: RoutineActionRequest = {id:`learning:${job.id}`,kind:"business_candidate_development",targetId:job.id,
...
kernel_new = '''      const decisionScope = `${state.standingMandate?.digest ?? "no-mandate"}:${now.slice(0,10)}`;
...
test_anchor = '''test("two consumers cannot duplicate a learning dispatch, including after restart", async()=>{'''
test_insert = r'''test("a replacement learning mandate does not reuse a stale daily-limit denial", async()=>{
...
''' + test_anchor
exact_replace("tests/autonomous-learning.test.ts", test_anchor, test_insert)
