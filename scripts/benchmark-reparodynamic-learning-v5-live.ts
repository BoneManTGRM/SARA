...
if (!process.argv.includes("--live")) throw new Error("The V5 benchmark requires an explicit --live flag.");
if (!process.argv.includes(SPEND_FLAG)) throw new Error(`The V5 benchmark requires ${SPEND_FLAG}.`);
const apiKey = process.env.OPENAI_API_KEY?.trim();
if (!apiKey) throw new Error("OPENAI_API_KEY is required.");
const sourceCommit = (process.env.SARA_BENCHMARK_COMMIT_SHA ?? process.env.GITHUB_SHA ?? process.env.RAILWAY_GIT_COMMIT_SHA ?? "").trim().toLowerCase();
if (!/^[a-f0-9]{40}$/u.test(sourceCommit)) throw new Error("Set SARA_BENCHMARK_COMMIT_SHA to the exact 40-character benchmark commit.");
...
const baseline: ProgramCandidateProposal = {
  ...
  programName: "Frozen allocation paid V5 benchmark",
  summary: "Synthetic public TypeScript fixture for a bounded paid Luna V5 trace.",
  limitations: ["Synthetic benchmark only. No repository, customer, deployment, or authority mutation."],
  files: [
    { path: "src/allocate.ts", content: [
    ...
    { path: "tests/allocate.test.ts", content: [
      ...
      'test("rejects invalid weights", () => { assert.throws(() => allocateCents(5, [0, 0]), RangeError); assert.throws(() => allocateCents(5, [-1, 2]), RangeError); });',
      "",
    ].join("\n") },
  ],
};
...
const context = { objective, acceptanceCriteria, missingCapabilities: [], constitutionDigest, memoryContext };
const result = await runMatchedCodingRepairBenchmarkV5({
  caseId: "allocate-cents-paid-v5-20260904",
  sourceCommit,
  modelRouteKey: client.routeKey,
  environment: { node: process.version, platform: process.platform, architecture: process.arch, typescript: ts.version },
  objective,
  acceptanceCriteria,
  constitutionDigest,
  memoryContextDigest: memoryContext.contextDigest,
  baseline,
  verify: async (candidate) => verifyGenomeLabProgramCandidate({ candidate, objective, acceptanceCriteria, constitutionDigest, maximumBudgetUsd: INITIAL_CODING_REPAIR_LIMITS.maximumModelSpendUsd }),
