...
import { KERNEL_BENCHMARK_PROTOCOL, assertKernelBenchmarkImplementation, runKernelCodingBenchmark } from "../src/exact-reuse-kernel-benchmark.ts";
import { KERNEL_BENCHMARK_PINS } from "../src/exact-reuse-kernel-pins.ts";
...
const sourceBindings = Object.fromEntries(await Promise.all(["src/exact-reuse-kernel-benchmark.ts", "src/exact-reuse-kernel-pins.ts", "scripts/benchmark-exact-reuse-kernel.ts",
  "src/exact-reuse-benchmark-grant.ts", "src/coding-benchmark-readiness.ts", "src/coding-benchmark-owner.ts", "src/coding-benchmark-evidence.ts", "src/coding-benchmark-github-relay.ts"]
