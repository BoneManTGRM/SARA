...
if (process.env.SARA_RUN_CODING_SPEED_BENCHMARK === "true") {
  ...
  if (/^[a-f0-9]{40}$/u.test(sourceRevision) && /^[A-Za-z0-9-]{8,}$/u.test(deploymentId)) {
    ...
    try {
      const { runSafeServiceRuntimeProof } = await import("../src/digital-capabilities/service-runtime-proof.ts");
      const proof = await runSafeServiceRuntimeProof({ kernel, sourceRevision, deploymentId, environment: "PRODUCTION" });
      console.log(JSON.stringify({ event: "sara_service_runtime_proof", ...proof }));
    } catch {
