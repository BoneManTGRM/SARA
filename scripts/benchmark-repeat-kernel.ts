...
import { KERNEL_BENCHMARK_PROTOCOL, assertKernelBenchmarkImplementation, runKernelCodingBenchmark } from "../src/repeat-kernel-benchmark.ts";
import { KERNEL_BENCHMARK_PINS } from "../src/repeat-kernel-pins.ts";
...
const beforeDispatch = async () => {
  ...
  // Separate new allowance. Historical consumed grants remain untouched and cannot authorize this CLI.
};
await beforeDispatch(); await assertKernelBenchmarkImplementation();
const sourceBindings = Object.fromEntries(await Promise.all(["src/repeat-kernel-benchmark.ts", "src/repeat-kernel-pins.ts", "scripts/benchmark-repeat-kernel.ts",
  "src/repeat-kernel-benchmark-grant.ts", "src/coding-benchmark-readiness.ts", "src/coding-benchmark-owner.ts", "src/coding-benchmark-evidence.ts", "src/coding-benchmark-github-relay.ts"]
