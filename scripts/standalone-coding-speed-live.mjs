...
function evaluate(source) {
  ...
  try {
    ...
    return { passed: false, score: 0, passedCount: 0, total: hiddenCases.length, failureCodes: ["SYNTAX_OR_LOAD"], digest: digest(source) };
  }
  let passedCount = 0;
  const failureCodes = [];
  for (let i = 0; i < hiddenCases.length; i += 1) {
    ...
    let actual;
    try { actual = fn(value, nowMs, maxMs); } catch { actual = Symbol("throw"); }
...
function extractSource(text) {
  ...
  const fence = candidate.match(/```(?:typescript|ts|javascript|js)?\\s*([\\s\\S]*?)```/i);
...
function controlPrompt(source, verification, cycle) {
...
async function run() {
  ...
  async function canaryArm() {
    ...
    let calls = 1;
    const lessons = [];
    if (!sharedVerification.passed) {
      lessons.push({ cycle: 1, scoreDelta: sharedVerification.score - baselineVerification.score, failureClasses: sharedVerification.failureCodes, tactics: tacticSignals(baseline, shared.source), outcome: sharedVerification.score > baselineVerification.score ? "retained_gain" : "rolled_back" });
    }
    for (let cycle = 2; cycle <= 3 && !verification.passed; cycle += 1) {
  ...
  const result = {
    ...
    control: {
      ...
      activeModelMilliseconds: Math.round(control.elapsedMs),
      logicalModelCalls: control.calls,
      inputTokens: control.inputTokens,
      outputTokens: control.outputTokens,
      finalSourceDigest: digest(control.source)
    },
    canary: {
      verifiedComplete: canary.verification.passed,
      score: canary.verification.score,
      activeModelMilliseconds: Math.round(canary.elapsedMs),
      logicalModelCalls: canary.calls,
      inputTokens: canary.inputTokens,
      outputTokens: canary.outputTokens,
      finalSourceDigest: digest(canary.source),
