...
export function enforceEffectBoundary(input:{
  ...
  effect:'READ'|'INTERNAL'|'EXTERNAL'|'PROHIBITED'|'UNKNOWN';
  decision:PolicyDecision;authority:'INTERNAL_POLICY'|'AUTHENTICATED_OWNER'|'EXACT_MANDATE'|null;
  ...
  if(decision.allowed){
    ...
    else if(input.effect==='EXTERNAL'&&(!input.authorityIdentity||!['AUTHENTICATED_OWNER','EXACT_MANDATE'].includes(input.authority??'')))decision={allowed:false,code:'EXACT_AUTHORITY_REQUIRED',reason:'An external effect requires the existing exact owner or mandate executor authority.'};
