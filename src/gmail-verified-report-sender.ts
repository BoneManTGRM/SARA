...
export class GmailVerifiedReportSender {
  ...
  async verifyIdentity(): Promise<void> { await this.#verifiedAccessToken(); }

  async #verifiedAccessToken(): Promise<string> {
  ...
  async send(inputValue: GmailVerifiedReportInput): Promise<GmailDeliveryReceipt> {
    ...
    // Re-read under the lock: another sender may have completed since the first read.
    try {
      await readFile(statePath, "utf8");
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
        await lock.close();
        await import("node:fs/promises").then(({ rm }) => rm(lockPath));
        throw error;
      }
      // Only a missing durable state permits a first send.
      return this.#sendLocked(input, digest, statePath, lockPath, lock);
    }
    await lock.close();
    await import("node:fs/promises").then(({ rm }) => rm(lockPath));
    throw new Error("A delivery state appeared during admission; automatic resend is refused.");
  }

  async #sendLocked(input: ReturnType<typeof validateInput>, digest: string, statePath: string, lockPath: string, lock: import("node:fs/promises").FileHandle): Promise<GmailDeliveryReceipt> {
