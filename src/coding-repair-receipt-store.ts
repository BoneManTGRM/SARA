...
export async function persistCodingRepairRun(input: {
  ...
  const summary = {
    ...
    baselineVerificationFresh: input.run.baselineVerificationFresh !== false,
    finalScore: input.run.verification.score,
    verifiedAccuracyGain: input.run.baselineVerificationFresh === false ? null : input.run.verification.score - input.run.baselineVerification.score,
