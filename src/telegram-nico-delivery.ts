...
import { mkdir, open, readFile, readdir, rename, writeFile, rm } from "node:fs/promises";
import { join } from "node:path";
import { verifyProductionNicoPackage } from "./nico-automated-package-verifier.ts";
import { extractNicoArtifactIdentity, type NicoOperator, type NicoArtifactIdentity } from "./nico-operator.ts";
...
type NicoBoundary = Pick<NicoOperator, "createRun" | "getRun" | "continueRun" | "getReviewQueue" | "getAutomatedDeliveryPackage">;
...
function verifyRun(value: unknown, expected: { runId: string; repositoryUrl: string; commitSha: string }, costLimitUsd: number): RunEvidence {
  ...
  if (FAILED.test(status) && status !== "review_required") throw new Error(`NICO stopped without an authorized automated package: ${status}.`);
  const explicitTerminal = firstValue(value, new Set(["terminal", "isterminal", "complete", "completed"])) === true;
  const terminal = explicitTerminal && (status === "review_required" || TERMINAL.has(status.toLowerCase().replace(/[ —-]+/gu, "_")));
  ...
  return { runId, repositoryUrl: canonicalRepository(repositoryUrl.startsWith("https://") ? repositoryUrl : `https://github.com/${repositoryUrl}`).repositoryUrl, commitSha, status, terminal, artifactIdentity: extractNicoArtifactIdentity(value, expected.runId) ?? undefined, costUsd };
}
...
function unresolvedReviewWorkload(value: unknown): number {
  ...
  const direct = optionalNumber(value, [
    "total_unresolved_human_review_work_units", "unresolved_review_workload", "unresolvedReviewWorkload", "unresolved_count", "unresolvedCount",
...
export class TelegramNicoDeliveryOperator {
  ...
  async #assertAuthority(workflow: WorkflowState, action: TelegramNicoAction): Promise<void> {
    const decision = await this.#authorize({ action, requestId: workflow.assessmentRequestId, assessmentRequestId: workflow.assessmentRequestId });
    if (!decision.allowed) throw new Error(decision.reason);
  }

  async #startRun(workflow: WorkflowState): Promise<WorkflowState> {
    if (workflow.stage !== "target_locked") return workflow;
    await this.#assertAuthority(workflow, "nico_assessment_start");
    await this.#nico.createRun({
      runId: workflow.runId,
      repository: workflow.repository,
      commitSha: workflow.commitSha,
      clientName: "Cody — private owner assessment",
      projectName: `SARA assessment ${workflow.assessmentRequestId}`.slice(0, 160),
      authorizedBy: "Cody via authenticated SARA Telegram mandate",
      authorizationScope: "Defensive read-only assessment of this exact public repository commit; private report to the owner only; no human-review claim.",
      primaryTechnicalContact: "SARA automated operator",
  ...
  async #refresh(workflow: WorkflowState): Promise<WorkflowState> {
    const run = verifyRun(await this.#nico.getRun(workflow.runId), workflow, this.#assessmentCostLimitUsd);
    if (workflow.stage === "package_verified" || workflow.stage === "provider_accepted") {
      if (!run.terminal || JSON.stringify(run.artifactIdentity) !== JSON.stringify(workflow.artifactIdentity)) throw new Error("The terminal artifact changed after package verification.");
      return workflow;
    }
    const next: WorkflowState = {
      ...workflow,
  ...
  async #continue(workflow: WorkflowState): Promise<WorkflowState> {
    ...
    if (workflow.stage !== "running") throw new Error("An uncertain assessment requires explicit recovery before continuation.");
    await this.#assertAuthority(workflow, "nico_assessment_continue");
  ...
  async #verifyPackage(workflow: WorkflowState): Promise<WorkflowState> {
    ...
    await this.#assertAuthority(workflow, "nico_automated_package");
    const raw = await this.#nico.getAutomatedDeliveryPackage(workflow.runId, undefined, {
      expectedArtifactIdentity: workflow.artifactIdentity,
      confirmExactArtifact: true,
      confirmAutomatedDisclosure: true,
    });
    const verified = verifyProductionNicoPackage(raw, {
      runId: workflow.runId,
      repositoryUrl: workflow.repositoryUrl,
      commitSha: workflow.commitSha,
  ...
  async #email(workflow: WorkflowState, command: TelegramNicoCommand): Promise<WorkflowState> {
    ...
    await this.#assertAuthority(workflow, "email_verified_report");
  ...
  async submit(inputValue: TelegramNicoCommand): Promise<TelegramNicoReceipt> {
    ...
    await this.#prepare();
    const lockPath = join(this.#directory, "admission.lock");
    const lock = await open(lockPath, "wx", 0o600).catch(() => { throw new Error("A Telegram NICO command is in progress or requires recovery; automatic takeover is refused."); });
    try {
      return await this.#submitExclusive(input);
    } finally {
      await lock.close();
      await rm(lockPath);
    }
  ...
  async #submitExclusive(input: TelegramNicoCommand & { action: TelegramNicoAction }): Promise<TelegramNicoReceipt> {
    ...
    if (existing) {
      ...
      throw new Error("A prior command has an uncertain outcome; automatic replay is refused.");
    } else {
    ...
    try {
      ...
      if (input.action === "nico_assessment_start") {
        if (!this.#gmailSender) throw new Error("SARA Gmail is not authenticated for verified report delivery.");
        await this.#gmailSender.verifyIdentity();
        ...
        workflow = await maybeJson<WorkflowState>(this.#workflowPath(assessmentRequestId)) ?? {
          ...
          runId: `comprun_${sha256(`sara-telegram-nico:${assessmentRequestId}`).slice(0, 32)}`,
          stage: "accepted",
          costUsd: 0,
          updatedAt: this.#now().toISOString(),
        };
