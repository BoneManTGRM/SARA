...
import { createObservedReuseBudget as createReuseSpeedBudget } from "./observed-reuse-benchmark.ts";
import { REUSE_SPEED_ARMS, type ReuseSpeedArm } from "./reuse-speed-benchmark.ts";
...
import { KERNEL_BENCHMARK_PINS } from "./exact-reuse-kernel-pins.ts";
...
export async function runKernelCodingBenchmark(input: {
  ...
  if ((input.executionKind === "scripted_offline" && !input.fetchImpl) ||
      (input.executionKind === "live" && (input.fetchImpl || input.benchmarkId !== EXACT_REUSE_BENCHMARK_GRANT.benchmarkId))) throw new Error("KERNEL_BENCHMARK_EXECUTION_KIND");
