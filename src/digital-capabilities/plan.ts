...
const candidateStep=(c:CapabilityContract,input:Json)=>c.id==='experience-to-procedure-compiler'&&input!==null&&typeof input==='object'&&!Array.isArray(input)&&input.sourceKind==='ISOLATED_REPAIR_RECEIPT';
function boundedEffect(c:CapabilityContract,input:Json):boolean{return ((['isolated-defect-reproducer','isolated-defect-repairer','software-journey-tester'].includes(c.id)||candidateStep(c,input))&&c.effect==='INTERNAL_STATE'&&c.authorityClass==='REVERSIBLE_AUTHORIZED')||['PURE','READ_ONLY','DRAFT_ONLY'].includes(c.effect)||(c.id==='commitment-tracker'&&c.effect==='INTERNAL_STATE'&&c.authorityClass==='READ_ONLY');}
function boundedAuthority(c:CapabilityContract,input:Json):boolean{return ['READ_ONLY','DRAFT_ONLY'].includes(c.authorityClass)||((['isolated-defect-reproducer','isolated-defect-repairer','software-journey-tester'].includes(c.id)||candidateStep(c,input))&&c.authorityClass==='REVERSIBLE_AUTHORIZED');}
...
export async function executeBoundedPlan(plan:CapabilityPlan,adapter:{contract:(id:string)=>Promise<CapabilityContract|undefined>;invoke:(request:CapabilityInvocation)=>Promise<CapabilityResult>;stopped:()=>Promise<boolean>;valuation?:(digest:string)=>Promise<number|null|undefined>},maximumSteps:number) {
 ...
 while(pending.length){
  ...
  for(const candidate of ready){const c=await adapter.contract(candidate.capabilityId);
   let reason=candidate.dependsOn.some(d=>blocked.some(b=>b.stepId===d))?'FAILED_PREREQUISITE':!c||c.contractDigest!==candidate.contractDigest||c.status!=='ENABLED'||c.qualification.status!=='PASSED'||!boundedEffect(c,candidate.input)||!boundedAuthority(c,candidate.input)?'CURRENT_QUALIFIED_BOUNDED_CONTRACT_REQUIRED':null;
  ...
  if(!contract||contract.contractDigest!==step.contractDigest||contract.status!=='ENABLED'||contract.qualification.status!=='PASSED'||!boundedEffect(contract,step.input)||!boundedAuthority(contract,step.input))return finish('BLOCKED',step.id,'CURRENT_QUALIFIED_BOUNDED_CONTRACT_REQUIRED');
