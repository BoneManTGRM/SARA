...
export function capabilityResult(input:{requestId:string;capabilityId:string;inputDigest:string;contract?:CapabilityContract;context:ExecutionContext;
  ...
  const unsigned={schemaVersion:1 as const,requestId:input.requestId,status:input.status??result.status??"SUCCEEDED",
    ...
    cost:{actualCashMicroUsd:0 as const,modelApiMicroUsd:0 as const,allocatedCashMicroUsd:0 as const,modeledLaborMicroUsd:null,
      costBasis:((contract?.id==='software-source-inspector'||contract?.id==='software-journey-tester')&&result.observed?.length?'BOUNDED_EXTERNAL_READ_NO_PAID_MODEL':contract?.id==='nico-production-proof-runner'&&result.observed?.some(o=>o&&typeof o==='object'&&!Array.isArray(o)&&o.basis==='ACTUAL_NICO_GET_RUN')?'BOUNDED_EXTERNAL_READ_NO_PAID_MODEL':'DETERMINISTIC_LOCAL_COMPUTATION_NO_EXTERNAL_CALL') as CapabilityResult['cost']['costBasis']},
