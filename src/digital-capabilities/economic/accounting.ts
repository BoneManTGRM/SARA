...
export function buildAuthoritativeJobAccounting(jobIds:string[],ledger:readonly LedgerEntry[],jobs:readonly RevenuePilotJob[]):AuthoritativeJobAccounting {
 ...
 const entries:Data[]=[],unknowns=['Refunds and per-job allocated hosting/tooling/labor are not structured in the existing ledger; absence of records does not prove zero expense.','Model expense uses conservatively accounted durable role costs, including failed attempts; these are not independent provider invoice attestations.'];
 ...
 const unattributed=ledger.filter(e=>!boundRevenueIds.has(e.id)).map(e=>({id:e.id,kind:e.kind,source:e.source,amountMicroUsd:micro(e.amountUsd),realized:e.realized,recurringMonthly:e.recurringMonthly}));
 ...
 const basisDigest=sha256(canonicalJson({jobIds:ids,entries,missingJobIds,unattributed,jobStatuses,aggregates:selected.map(j=>({id:j.id,actualExecutionCostMicroUsd:micro(j.actualExecutionCostUsd)}))}));
