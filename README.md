...
- an owner-controlled self-build cycle that invokes a replaceable candidate generator and accepts either a small pure `runSkill(input)` module or a complete bounded multi-file TypeScript program;
- a program-builder boundary that requires `src/index.ts`, multiple source modules, independent test modules, dependency-free relative imports, strict type checking, and a restricted test process with no network, child-process, or filesystem-write permission;
- durable job states and an exhaustive candidate digest, with successful self-built skills and programs stopping automatically at SHADOW and failed candidates leaving an immutable audit outcome instead of a production change.
...
Before creating an invoice, the public site may call `POST /api/public/revenue-pilot/preflight` with one canonical public GitHub repository URL, one supported goal, repository-authority confirmation, and explicit answers to every excluded-scope question. SARA performs one anonymous, read-only repository lookup and returns the fixed deliverables, exclusions, fit decision, evidence gaps, and safest next step. The preflight creates no job, reservation, payment intent, revenue record, work authorization, or delivery authority. Public preflight and payment-intent attempts share the existing five-per-hour client limit.
...
The built-in coding system is deliberately bounded. It now proves the complete safe takeover seam—objective → generator → multi-file source → compiler → isolated tests → hashed artifact → durable SHADOW—but it is not an unrestricted unsupervised product developer. A model-backed executor can plug into the same `CandidateGenerator` contract without receiving secrets, production access, payment authority, or promotion authority. The owner API accepts a bounded skill or program proposal at `POST /api/jobs/:id/self-build`; this is authenticated and can never promote beyond SHADOW. Program candidates are limited to 24 files and 48 KiB, use no external dependencies, and cannot request filesystem, network, process, timer, or dynamic-code authority. CANARY and later stages require a locally re-verifiable artifact, kernel-executed evidence, and stage-specific approval from the authenticated constitutional owner. The exhaustive artifact tree—including paths, exact bytes, file types, and permissions—is re-hashed immediately before every promotion.
...
The deliberate stopping point is the safe takeover boundary: SARA can accept, write, test, and reject a bounded pure skill or multi-file TypeScript program and publish verified source as a reviewable draft PR. She cannot edit the champion, install arbitrary packages, use credentials, merge, deploy, spend without an earned budget, or open financial accounts. Any broader coding capability must pass the same work-card, Genome Lab, verification, draft-only, and owner-promotion boundaries. No bank, paid API, new account, merge, or production deployment is silently provisioned.

### Free-model recovery path

If the site-to-executor queue is unavailable, an owner may manually dispatch **SARA free-model self-build** from GitHub Actions. The workflow requires a bounded objective plus an explicit public-draft checkbox. It makes one request to the fixed Cloudflare Workers AI model `@cf/zai-org/glm-4.7-flash` and, only when the resulting proposal is rejected after generation, permits one final repair request containing that rejected proposal. Both outputs are untrusted; the existing Genome Lab restrictions and behavioral verification still decide whether work stops as a draft `SHADOW` pull request or fails closed.

This recovery path is locked to a Workers **Free** account. Add `CLOUDFLARE_ACCOUNT_ID` and a restricted `CLOUDFLARE_API_TOKEN` as GitHub Actions repository secrets; never place either value in an objective, workflow input, issue, log, or committed file. Do not use this workflow after upgrading Workers: its `$0` guarantee depends on the Free plan failing closed when the daily allocation is unavailable. The workflow has no schedule, unbounded retries, paid overflow, merge, deployment, outreach, contract, account, payment, or production authority. A path-filtered repository authorization marker may activate one named objective once; unrelated future pushes cannot activate it.
