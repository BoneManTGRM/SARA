...
- a bounded digital-work control plane for software changes, software testing, documentation, localization, public research, data transformation, and repository assessments;
- two-step job qualification and exact owner acceptance, least-authority executor handoffs, deterministic verification records, mandatory human review where judgment is not machine-verifiable, separately authorized delivery, and owner-attested settled-payment accounting.
- a durable verified-skill registry that records generator provenance, candidate and mutation digests, behavioral tests, limitations, lifecycle stage, and execution evidence;
- owner-bound capability identities and a restricted JSON skill runtime: SHADOW and CANARY skills remain unusable, LIMITED PRODUCTION skills become callable, and changed or failing artifacts quarantine themselves and remove their capability from service.
...
The digital-work desk follows the same rule. It can compile and authorize seven safe job families and expose an exact executor handoff through the authenticated `/api/digital-jobs` routes. It does **not** pretend that an executor exists when none is installed. Each production executor must declare supported job kinds, remain at a $0 maximum cost, accept only the compiled tool envelope, return a hashed artifact plus successful verification records, and fail closed. Repository/security assessments always require human review during bootstrap; jobs with completely automatable acceptance criteria may stop at owner review-ready without human labor. Contracts, buyer contact, identity/KYC, external accounts, delivery, deployment, and money remain separate owner gates.

Self-built pure skills now complete a real learning loop. A successful zero-cost build becomes a durable SHADOW skill record, but it has no callable authority. The authenticated owner must bind the exact candidate to a stable capability id and approve each production stage. At LIMITED PRODUCTION the kernel may invoke that exact digest in a three-second, 64 MiB, no-network, no-child-process, bounded-JSON runtime. Every run records only input/output digests and success evidence in the immutable audit. Artifact tampering or a runtime failure quarantines the skill and marks its capability missing; recovery requires a new verified candidate rather than silently trusting the failed one.
