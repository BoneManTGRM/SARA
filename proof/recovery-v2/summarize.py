...
assert len(r['rows']) == len(frozen['rows']) == 135
...
for variant in 'ABC':
    ...
    # Count a successful recovery only after a post-baseline repair test fails.
    ...
    aggregate[variant] = dict(resolved=sum(x['resolved'] for x in rows),
        ...
        finishReconsiderations=decisions['reject_finish'],
...
for case, variant in [('affine-zero-invariant','B'),('affine-zero-invariant','C'),('no-alternative','C'),('ignores-rejection','C'),('public-pass-unresolved','C'),('uncertain-call','C')]:
