...
const tempSource=join(root,'src',`.recovery-v2-baseline-${process.pid}.ts`);
writeFileSync(tempSource,baseline,{flag:'wx'});
const currentSource=join(root,'src',`.recovery-v2-current-${process.pid}.ts`);
...
try {
  ...
  for(let rep=0;rep<settings.repetitions;rep++)for(const f of fixtures){
    ...
    for(const variant of order){
      ...
      const adapter=modelFor({hypotheses:structuredClone(f.hypotheses),wholeFile:!!f.wholeFile,ignoreRejection:!!f.ignoreRejection});
      const producer=await (variant==='C'?runRepositoryProducer:(variant==='B'?prior:original))({
...
}finally{rmSync(tempSource,{force:true});rmSync(currentSource,{force:true});rmSync(scratch,{recursive:true,force:true});}
