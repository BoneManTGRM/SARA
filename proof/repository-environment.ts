...
let resourcesBefore: string | null = null;
try {
  ...
  const resourcesAfter = await recordResources("after");
  if (!repositoryResourceLimitsRespected(resourcesBefore, resourcesAfter)) {
    receipt.publicTestsPassed = false;
    receipt.error ??= "RESOURCE_LIMIT_EXCEEDED_OR_EVIDENCE_UNAVAILABLE";
  }
