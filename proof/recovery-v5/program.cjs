...
function compute(x){
 if(family==='join')return c.uppercase?x.join(c.separator).toUpperCase():x.join(c.separator);
 if(family==='sort')return [...x].sort((a,b)=>c.descending?b-a:a-b).slice(0,c.limit).join(',');
