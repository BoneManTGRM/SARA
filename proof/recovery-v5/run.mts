...
const tempSource=join(root,'src',`.recovery-v5-baseline-${randomUUID()}.ts`);
writeFileSync(tempSource,baseline,{flag:'wx'});
const currentSource=join(root,'src',`.recovery-v5-current-${randomUUID()}.ts`);
...
const candidateSource=join(root,'src',`.recovery-v5-candidate-${randomUUID()}.ts`);writeFileSync(candidateSource,candidateBytes,{flag:'wx'});
  ...
  for(const row of rows){
    ...
    try{
      ...
      if(f.partialProgress){
        ...
        row.partialProgress={alignedStates:aligned,lossTransitions:losses,reusedAfterLoss:reuses,finalAligned:Object.entries(f.partialProgress).every(([k,v])=>JSON.stringify(JSON.parse(readFileSync(join(dir,'src/settings.json'),'utf8'))[k])===JSON.stringify(v)),
