...
const tempSource=join(root,'src',`.recovery-v3-baseline-${randomUUID()}.ts`);
writeFileSync(tempSource,baseline,{flag:'wx'});
const currentSource=join(root,'src',`.recovery-v3-current-${randomUUID()}.ts`);
...
const candidateSource=join(root,'src',`.recovery-v3-candidate-${randomUUID()}.ts`);writeFileSync(candidateSource,candidateBytes,{flag:'wx'});
...
try {
    ...
    for(const variant of order){
      ...
      const producer=await (variant==='C'?candidate:(variant==='B'?prior:original))({
        task:{instanceId:f.id,runId:`offline-${rep}-${f.id}-${variant}`,arm:variant==='A'?'conventional':'reparodynamic',
          problemStatement:JSON.stringify({instruction:'Repair the toy application. Explore the public finite configuration domain and use test observations.',searchSpace:f.space,wholeFile:!!f.wholeFile})},
  ...
  if(!baselineOnly){
    ...
    gate={losses,savings:savings.map((t:any)=>t.caseId),families:[...new Set(savings.map((t:any)=>t.family))],overhead,
    ...
    gate.accepted=gate.families.length>=2&&losses.length===0&&gate.noAddedRegressions&&gate.negativesRejected&&overhead.every((o:any)=>o.tools<=2&&o.tests<=2&&o.cMs<=o.allowedMs);
  }
