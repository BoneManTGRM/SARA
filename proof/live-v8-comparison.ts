...
type BenchmarkStage='baseline_verification'|'model_request'|'candidate_validation'|'candidate_verification'|'final_verification'|'unknown';
function boundedFailure(error:unknown,stage:BenchmarkStage){return {stage,code:error instanceof Error&&error.name?String(error.name).slice(0,120):'UNCLASSIFIED_ERROR'};}
...
const verify=async(candidate:ProgramCandidateProposal)=>{assert.deepEqual(candidate.files.filter(f=>f.path.startsWith('tests/')),baseline.files.filter(f=>f.path.startsWith('tests/')),'protected verifier changed');return verifyGenomeLabProgramCandidate({candidate,objective,acceptanceCriteria,constitutionDigest});};
const preflight={reference:await verify(reference),baseline:await verify(baseline)};
assert(preflight.reference.passed,'reference must pass');assert(!preflight.baseline.passed,'baseline must fail');
...
type Physical={id:number;arm:string;cycle:number;status:string;inputTokens:number|null;outputTokens:number|null;reasoningTokens:number|null;responseId:string|null;responseModel:string|null;httpStatus:number|null;httpMilliseconds:number|null;costUsd:number|null;upperBoundUsd:number;promptDigest:string};
...
const observedFetch:typeof fetch=async(url,init)=>{
 const target=String(url),generation=target==='https://api.openai.com/v1/responses';let receipt:Physical|undefined;let body:Record<string,any>|null=null;
 if(init?.body){try{body=JSON.parse(String(init.body));}catch{/* client handles malformed */}}
 if(generation){
  assert(physical.length<MAX_GENERATIONS,'physical generation ceiling exceeded');assert(physical.filter(r=>r.arm===activeArm).length<3,'per-arm generation ceiling exceeded');
  assert.equal(body?.model,'gpt-5.6-luna');assert.equal(body?.reasoning?.effort,'medium');assert.equal(body?.store,false);
  const maxOut=Number(body?.max_output_tokens);assert(Number.isInteger(maxOut)&&maxOut>0&&maxOut<=8000,'output-token ceiling');
  const reserve=(30000*0.25+maxOut*1.2)/1e6;assert(sumUpper()+reserve<=LIMIT&&sumUpper(activeArm)+reserve<=ARM_LIMIT,'physical spend reservation ceiling');
  receipt={id:physical.length+1,arm:activeArm,cycle:currentCycle,status:'attempted',inputTokens:null,outputTokens:null,reasoningTokens:null,responseId:null,responseModel:null,httpStatus:null,httpMilliseconds:null,costUsd:null,upperBoundUsd:reserve,promptDigest:sha256(String(body?.input??''))};physical.push(receipt);emit('V8_MODEL_ATTEMPT',receipt);
 }
 const start=performance.now();try{const response=await rawFetch(url,init);if(receipt){receipt.httpStatus=response.status;receipt.httpMilliseconds=performance.now()-start;const payload=await response.clone().json() as Record<string,any>;receipt.responseId=typeof payload.id==='string'?payload.id:null;receipt.responseModel=typeof payload.model==='string'?payload.model:null;const u=payload.usage;if(u&&Number.isSafeInteger(u.input_tokens)&&Number.isSafeInteger(u.output_tokens)&&u.input_tokens>=0&&u.output_tokens>=0){receipt.inputTokens=u.input_tokens;receipt.outputTokens=u.output_tokens;receipt.reasoningTokens=Number.isSafeInteger(u.output_tokens_details?.reasoning_tokens)?u.output_tokens_details.reasoning_tokens:null;receipt.costUsd=(u.input_tokens*0.20+u.output_tokens*1.20)/1e6;receipt.upperBoundUsd=(u.input_tokens*0.25+u.output_tokens*1.20)/1e6;}receipt.status=response.ok&&payload.status==='completed'?'completed':'provider_not_completed';emit('V8_MODEL_RECEIPT',receipt);}return response;}catch(error){if(receipt){receipt.status='unresolved_failure';receipt.httpMilliseconds=performance.now()-start;emit('V8_MODEL_FAILURE',receipt);}throw error;}
};
function modelClient():WorkerModelClient{
 ...
 return {routeKey:'openai:gpt-5.6-luna:paid',maximumWallTimeMs:1000,async countInputTokens(){return 100;},async execute(input){assert.equal(input.reasoningLevel,'medium');const facts=JSON.parse(input.prompt.split('\n').slice(2).join('\n'));const current=facts.files.find((f:{path:string})=>f.path==='src/inventory.ts');const compact=input.prompt.startsWith('OUTPUT CONTRACT: SARA_CODING_REPAIR_EDITS_V1');const edits=mutations.map(m=>({find:m.replace,replace:m.find}));return {outputText:JSON.stringify({schemaVersion:1,baseArtifactDigest:facts.currentArtifactDigest,failureFingerprint:facts.failures[0].fingerprint,strategy:facts.requiredStrategy,changes:[{path:current.path,expectedContentDigest:current.contentDigest,...(compact?{edits}:{replacementText:good})}],limitations:[]}),inputTokens:100,billableOutputTokens:100};}};
}
const arms:Array<Record<string,any>>=[];
for(const arm of contract.armOrder){
 activeArm=arm;const compact=arm==='compact_first',start=performance.now();let verificationMs=0,modelMs=0,attemptedModelCalls=0,verificationCalls=0;const stage:{value:BenchmarkStage}={value:'baseline_verification'};const receipts:CodingRepairReceipt[]=[];const requests:Array<Record<string,unknown>>=[];const adapter=createLunaCodingRepairModel({client:modelClient(),context,compactRepairContinuations:compact,experimentalCompactFirstProposal:compact});
