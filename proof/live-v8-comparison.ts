import {workerClient,finishWorker,workerEvidenceMode} from './benchmark-worker-client.ts';
...
import {baseline,reference,good,broken,mutations,objective,acceptanceCriteria,assertionCount} from './v8-live-fixture.ts';
...
assert(!(bridge&&selfTest),'bridge and self-test are mutually exclusive');
...
const supervision=await loadV8SupervisedContract();
const {digest:contractDigest,...contract}=supervision;
function emit(type:string,payload:unknown){if(!bridge)console.log(JSON.stringify({type,payload}));}
const sourceManifest=JSON.parse(await readFile('proof/live-v8-source-manifest.json','utf8')) as Record<string,string>;
...
if(!bridge&&!selfTest&&!process.argv.includes('--live')){await writeFile('/tmp/sara-v8-contract.json',JSON.stringify(contract,null,2));process.exit(0);}
...
function modelClient():WorkerModelClient{
 ...
 return {routeKey:'openai:gpt-5.6-luna:paid',maximumWallTimeMs:1000,
  async countInputTokens(prompt){assert(!prompt.includes('PRIVATE_V8_INVENTORY_ORACLE'));return 100;},
  async execute(input){
   ...
   const current=facts.files.find((f:{path:string})=>f.path==='src/inventory.ts');
   const compact=input.prompt.startsWith('OUTPUT CONTRACT: SARA_CODING_REPAIR_EDITS_V1');
   const wrongFind='return [line,...lines].map(copy);';
   const fixes=allWrong?[{find:wrongFind,replace:wrongFind+` // scripted wrong ${currentCycle}`}]:mutations.map(m=>({find:m.replace,replace:m.find}));
   const replacementText=allWrong?broken.replace(wrongFind,wrongFind+` // scripted wrong ${currentCycle}`):good;
...
for(const arm of contract.armOrder){
 ...
 try{
   ...
   onReceipt:r=>{receipts.push(structuredClone(r));emit('ARM_RECEIPT',{arm,receipt:r});},model:{
    async propose(request){
     ...
     try{const output=await adapter.propose({...request,remainingCostUsd:Math.min(request.remainingCostUsd,0.03)});
      stage.value='candidate_validation';requests.push({cycle:request.cycle,inputDigest,proposalDigest:sha256(canonicalJson(output.proposal)),proposal:output.proposal,elapsedMilliseconds:performance.now()-t});return output;
  ...
  result={arm,verifiedComplete:post.passed,score:post.score,timeMs:performance.now()-start,modelMilliseconds:modelMs,verificationMilliseconds:verificationMs,
   ...
   finalSource:run.champion.files.filter(f=>f.path.startsWith('src/')),finalVerification:post,error:null,failure:null};
...
const measuredLive=bridge&&workerEvidenceMode()==='live';
// Synthetic timing is never a live speed measurement. Preserve only mechanism success/failure.
if(!measuredLive){evaluation.timeComparable=false;evaluation.speedRatio=null;evaluation.speedIncreasePercent=null;evaluation.target300PercentMet=false;evaluation.costNotHigher=null;evaluation.verdict=evaluation.valid?'OFFLINE_INTEGRATION_ONLY':'INCONCLUSIVE';}
const record={contract,contractDigest,sourceCommit:SOURCE,supervisionContractDigest:supervision.digest,evidenceLevel:measuredLive?'LIVE_V8_SUPERVISED_COMPARISON':bridge?'OFFLINE_SUPERVISED_V8_VERIFIER':'SCRIPTED_V8_HARNESS_SELF_TEST',
 ...
 preflight,environment:{node:process.version,platform:process.platform,arch:process.arch,railwayDeploymentId:process.env.RAILWAY_DEPLOYMENT_ID??null,actualCommit:process.env.RAILWAY_GIT_COMMIT_SHA??null},generalClaimSupported:false};
...
else await writeFile('/tmp/sara-v8-comparison-result.json',text+'\n',{mode:0o600});
