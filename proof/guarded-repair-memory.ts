...
function key(candidate:ProgramCandidateProposal,scope:Scope):string{
 if(Object.keys(scope).length!==4||!Object.values(scope).every(v=>typeof v==='string'&&/^[a-f0-9]{64}$/u.test(v)))throw Error('INVALID_RECIPE_SCOPE');
...
export class GuardedRepairMemory {
 ...
 learn(before:ProgramCandidateProposal,after:ProgramCandidateProposal,verification:ProgramVerificationResult,scope:Scope):string{
  if(!verification.passed||verification.score!==1||verification.failures.length||verification.artifactDigest!==artifact(after)||
   !['source_policy','syntax','typecheck','behavior_tests','artifact_integrity'].every(x=>verification.completedChecks.includes(x as any))||
   !verification.evidenceDigests.length)throw Error('UNVERIFIED_RECIPE');
  if(before.files.length!==after.files.length||new Set(after.files.map(f=>f.path)).size!==after.files.length)throw Error('RECIPE_FILE_SET_CHANGED');
