...
import { EXACT_REUSE_BENCHMARK_GRANT as consumed } from "../src/exact-reuse-benchmark-grant.ts";
import { REPEAT_KERNEL_BENCHMARK_GRANT as grant } from "../src/repeat-kernel-benchmark-grant.ts";
...
test("repeat run has a fresh capped grant while previous activation and unresolved hold remain unchanged", () => {
  ...
  for (const older of [consumed, previous, observed]) assert.equal(activeCodingBenchmarkContinuation({
...
test("repeat grant requires current canary, owner, constitution and stop authority", () => {
...
test("repeat launcher routes to its own CLI with bound caps, preserving prior CLI routes", () => {
  ...
  assert.equal(spec.args[2], "scripts/benchmark-repeat-kernel.ts");
  ...
  for (const [older, path] of [[consumed, "scripts/benchmark-exact-reuse-kernel.ts"], [previous, "scripts/benchmark-kernel-coding.ts"], [observed, "scripts/benchmark-observed-reuse.ts"]] as const) {
...
test("repeat evidence exports only bounded kernel jobs and traces, retaining historical isolation", () => fixture(async root => {
...
for (const throws of [false, true]) {
  test(`repeat execution claim prevents replay after ${throws ? "failure" : "success"}`, () => fixture(async root => {
