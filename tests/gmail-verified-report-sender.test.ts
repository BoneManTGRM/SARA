...
describe("verified Gmail report sender", () => {
  it("verifies exact sender immediately before sending and preserves the verified attachment bytes", async () => {
    ...
    const sender = new GmailVerifiedReportSender({
      ...
      fetchImpl: async (input, init) => {
        ...
        calls.push({ url, init });
        if (new URL(url).hostname === "oauth2.googleapis.com") return Response.json({ access_token: "access-token-value", token_type: "Bearer" });
        if (new URL(url).hostname === "openidconnect.googleapis.com") return Response.json({ email: GMAIL_REPORT_SENDER, email_verified: true });
  ...
  it("rejects a non-SARA authenticated account and never calls Gmail send", async () => {
    ...
    assert.equal(urls.some((url) => new URL(url).hostname === "gmail.googleapis.com"), false);
  });
