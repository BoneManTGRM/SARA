...
test('reachability covers the live inventory and excludes trusted owner controls from ordinary tools',()=>fixture(async({kernel})=>{
 ...
 assert.deepEqual(matrix.map(r=>r.id),contracts.map(c=>c.id));assert.equal(matrix.length,98);
...
test('exact Nico owner phrase preserves passing observations, scope and next diagnostic without inventing a repair blocker',()=>fixture(async({request})=>{
 const result=await (await request({requestId:'nico-no-defect',text:'Review this software defect analysis for Nico’s World and give me a brief with findings and the next diagnostic step.',suppliedText:nicoReport.replace('Unknown:', 'Evidence: '+ 'bounded browser observation. '.repeat(25)+'\nUnknown:')})).json() as any;
