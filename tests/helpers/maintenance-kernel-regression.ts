// Test-only current-kernel regression harness derived from the frozen repeat runner.
// Historical runner, pins, grants and live results remain byte-identical.
// Only injected fixture responses and a fixed non-secret key are accepted.
// This exercises fresh HTTP/kernel acceptance on evolving source; it is not a live benchmark.
import assert from "node:assert/strict";
import { KERNEL_BENCHMARK_PINS as historicalSourcePaths } from "../../src/repeat-kernel-pins.ts";
...
import { CodingDispatchJournal, type DispatchCounters } from "../../src/coding-dispatch-journal.ts";
import { NativeCodingVerifier } from "../../src/native-coding-verifier.ts";
import { createBenchmarkAudit, writeBenchmarkAudit } from "../../src/coding-benchmark-audit.ts";
import { createObservedReuseBudget as createReuseSpeedBudget } from "../../src/observed-reuse-benchmark.ts";
import { REUSE_SPEED_ARMS, type ReuseSpeedArm } from "../../src/reuse-speed-benchmark.ts";
...
import { REPEAT_KERNEL_BENCHMARK_GRANT } from "../../src/repeat-kernel-benchmark-grant.ts";
...
export const KERNEL_BENCHMARK_PROTOCOL = Object.freeze({
  ...
  classification: "SCRIPTED_OFFLINE_CURRENT_KERNEL_REGRESSION_NOT_LIVE_EVIDENCE",
  ...
  optimization: "Exact full-candidate and scope reuse; fresh authoritative final overlaps independent kernel worker; no old PASS authority.",
...
/** Runs the current server/kernel in isolated test state with injected fixture responses.
 * It cannot activate a paid grant and never supplies real model credentials. */
export async function runKernelCodingBenchmark(input: {
  ...
  if(input.executionKind!=="scripted_offline"||!input.fetchImpl||input.apiKey!=="SCRIPTED_NEVER_LIVE")throw new Error("OFFLINE_REGRESSION_ONLY");
  const currentSourceHashes=Object.fromEntries(await Promise.all(Object.keys(historicalSourcePaths).map(async path=>[path,sha256(await readFile(new URL(`../../${path}`,import.meta.url)))])));
  ...
  try {
    await writeBenchmarkAudit(trace, "kernel-registration.json", { benchmarkId: input.benchmarkId,
      protocol: KERNEL_BENCHMARK_PROTOCOL, task, taskDigest: sha256(canonicalJson(task)), sourcePins: currentSourceHashes,
      ...
      for (const arm of order) {
        ...
        try {
          ...
          try {
            ...
            assert(value.timing.totalMilliseconds <= elapsedMilliseconds); assert.equal(value.timing.pooled, round > 0 && arm !== "regenerate");
    ...
    const summary = { protocol: KERNEL_BENCHMARK_PROTOCOL, benchmarkId: input.benchmarkId, executionKind: input.executionKind,
      rows, aggregates, allComplete, comparisonAllowed: allComplete, sourcePins: currentSourceHashes,
