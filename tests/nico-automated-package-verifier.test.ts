...
it("verifies the production ZIP and rejects changed targets, source artifacts, and bytes", async () => {
 const {productionPackage}=await import("./nico-production-package-fixture.ts");
 const {verifyProductionNicoPackage}=await import("../src/nico-automated-package-verifier.ts");
 const runId=`comprun_${"1".repeat(32)}`; const p=productionPackage(runId,COMMIT);
 const expected={runId,repositoryUrl:REPOSITORY,commitSha:COMMIT,artifactIdentity:p.identity};
 assert.equal(verifyProductionNicoPackage(p,expected).reportDigest,p.digest);
 assert.throws(()=>verifyProductionNicoPackage(p,{...expected,commitSha:"f".repeat(40)}),/immutable target/);
 assert.throws(()=>verifyProductionNicoPackage(p,{...expected,artifactIdentity:{...p.identity,report_artifact_digest:"e".repeat(64)}}),/source artifact/);
 const body=new Uint8Array(p.body);body[50]^=1;
 assert.throws(()=>verifyProductionNicoPackage({...p,body},expected),/digest/);
 assert.throws(()=>verifyProductionNicoPackage({...p,body,digest:sha256(body)},expected));
});
