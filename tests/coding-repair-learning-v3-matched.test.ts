...
describe("matched information-dense Reparodynamic learning v3", () => {
  it("improves verified completion on a fresh deterministic holdout without authority expansion", async () => {
    ...
    assert.equal(result.contract.schemaVersion, 3);
    assert.equal(result.contract.canaryPolicy, "bounded_reparodynamic_information_learning_v3");
    assert.equal(
    ...
    assert.equal(result.authority.maximumModelSpendUsd, INITIAL_CODING_REPAIR_LIMITS.maximumModelSpendUsd);
    ...
    assert(result.canary.modelAttemptLessons.some((lesson) => (
      lesson.outcome === "rolled_back" && lesson.sourceSignals.includes("call:Math.round:+1")
    )));
    assert(result.control.modelAttemptLessons.every((lesson) => lesson.sourceSignals.length === 0));
    ...
    assert.match(result.modelAttemptLessonsDigest, /^[a-f0-9]{64}$/u);
    assert.match(result.learningEvidenceDigest, /^[a-f0-9]{64}$/u);
