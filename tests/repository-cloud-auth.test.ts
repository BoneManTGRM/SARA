...
const permit: RepositoryCloudPermit = { schemaVersion: 1, benchmarkId: "cloud-fixture", registrationDigest: "a".repeat(64),
  runtimeRevision: "b".repeat(40), workflowRevision: "b".repeat(40), workflowRef: "refs/heads/cloud-fixture",
...
const claims = { iss: "https://token.actions.githubusercontent.com", aud: CLOUD_AUDIENCE,
  sub: "repo:BoneManTGRM/SARA:ref:refs/heads/cloud-fixture", repository: "BoneManTGRM/SARA", repository_id: "1313793559",
  ...
  workflow_sha: permit.workflowRevision, sha: permit.workflowRevision, event_name: "workflow_dispatch", run_attempt: "1",
...
test("signed fork, source, role, event, rerun, time and owner substitutions cannot authenticate", async () => {
  ...
  for (const change of [{ aud: "owner" }, { repository_id: "2" }, { actor_id: "2" }, { repository_owner_id: "2" },
    ...
    { event_name: "push" }, { event_name: "pull_request_target" }, { run_attempt: "2" }, { runner_environment: "self-hosted" },
