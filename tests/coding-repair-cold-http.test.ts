...
test("concurrent authorized HTTP jobs share one cold model generation but get separate kernel evidence", async () => {
  ...
  try {
    ...
    for (let turn = 0; turn < 1; turn++) {
      ...
      const jobs = [];
      for (let n = 0; n < 4; n++) jobs.push(await kernel.createSelfDevelopmentJob(SARA_PRINCIPAL, {
        objective: "Repair the value to 17", expectedOwnerValue: 1, requiredCapabilities: [],
        acceptanceCriteria: ["Protected test reads 17"], maximumBudgetUsd: 0.15,
      }));
      ...
      try {
        ...
        const responses = await Promise.all(jobs.map(job => fetch(`${base}/api/jobs/${job.id}/self-build`, {
          method: "POST", headers: { authorization: `Bearer ${token}`, "content-type": "application/json" }, body: data,
        })));
        const evidenceIds = new Set<string>(), mutationIds = new Set<string>();
        for (const response of responses) {
          ...
          if (!digest) digest = result.mutation.candidateDigest;
          assert.equal(result.mutation.candidateDigest, digest);
          evidenceIds.add(result.evidence.id); mutationIds.add(result.mutation.id);
        }
        assert.equal(evidenceIds.size, 4); assert.equal(mutationIds.size, 4);
    ...
    assert.equal(summaries.filter(s => s.hits === 1 && s.modelRequests === 0).length, 3);
