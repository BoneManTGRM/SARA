...
test("queued deadline prevents delayed dispatch while active work drains normally", async (t) => {
  t.mock.timers.enable({apis:["setTimeout"]});
  const q=new KernelBuildQueue();let release!:()=>void;const wait=new Promise<void>(r=>{release=r;});
  const a=q.run(async()=>{await wait;}),b=q.run(async()=>{await wait;});let called=false;
  try {
    const rejected=assert.rejects(q.run(async()=>{called=true;}),/DEADLINE/);
    assert.deepEqual(q.snapshot(),{active:2,queued:1,closed:false,maximumActive:2});
    t.mock.timers.tick(30_000);
    await rejected;assert.equal(called,false);
    assert.deepEqual(q.snapshot(),{active:2,queued:0,closed:false,maximumActive:2});
  } finally { release();await Promise.all([a,b]);await q.close(); }
});
