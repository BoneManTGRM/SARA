...
test('all 93 owner-requested capabilities have reviewed qualified contracts and execution boundaries',async()=>{
 ...
 const contracts=await capabilityContracts();assert.deepEqual(contracts.map(c=>c.id).sort(),[...expected,'isolated-defect-reproducer','software-source-inspector','software-journey-tester','software-evidence-reviewer'].sort());
